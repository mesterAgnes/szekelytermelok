...
def termekbetoltes():
	...
	cursor.execute(select_penznem, [session['SZ_ID']])
	cnx.commit()
	penznem = cursor.fetchone()
	logging.warning(penznem)
	
	cursor.close()
	cnx.close()
	return jsonify({'termekek':termekek, 'mertekegysegek':mertekegysegek, 'kategoriak':kategoriak, 'penznem': penznem})
...
def mindentermek():
	...
	select_promok = ("SELECT t.T_ID, t.Nev, pr.Ar, t.Kep, sz.Nev, p.Penznem, Keszlet_menny "
					"FROM Termekek t, Szemelyek sz, Termelok te, Penznemek p, Promociok pr "
					"WHERE sz.SZ_ID=t.SZ_ID and sz.SZ_ID=te.SZ_ID AND p.P_ID=te.P_ID and pr.T_ID=t.T_ID and t.T_ID IN (SELECT T_ID FROM promociok)")
	cursor.execute(select_promok)
	cnx.commit()
	promok = cursor.fetchall()
		
	# a NEM promocios termekek lekerdezese: 
	select_nempromok =  ("SELECT T_ID, t.Nev, t.Ar, t.Kep, sz.Nev, p.Penznem, Keszlet_menny "
						"FROM Termekek t, Szemelyek sz, Termelok te, Penznemek p "
						"WHERE sz.SZ_ID=t.SZ_ID and sz.SZ_ID=te.SZ_ID AND p.P_ID=te.P_ID and T_ID NOT IN (SELECT T_ID FROM Promociok)")
...
def egytermek():
	...
	select_termek = ("SELECT t.Nev, t.Leiras, t.Ar, t.Kep, t.Min_rendelesi_menny, t.Keszlet_menny, sz.Nev, p.Penznem, k.Nev, m.Nev FROM Termekek t, Szemelyek sz, Termelok te, Penznemek p, Kategoriak k, Mertekegysegek m WHERE sz.SZ_ID=t.SZ_ID and te.SZ_ID=sz.SZ_ID and p.P_ID=te.P_ID and k.K_ID=t.K_ID and m.ME_ID=t.ME_ID and T_ID = %s")
...
def egypromtermek():
	...
	select_termek = ("SELECT t.Nev, t.Leiras, t.Ar, t.Kep, t.Min_rendelesi_menny, t.Keszlet_menny, sz.Nev, p.Penznem, k.Nev, m.Nev, pr.Ar, pr.Periodus_k, pr.Periodus_v "
					 "FROM Termekek t, Szemelyek sz, Termelok te, Penznemek p, Kategoriak k, Mertekegysegek m,Promociok pr "
					 "WHERE sz.SZ_ID=t.SZ_ID and sz.SZ_ID=te.SZ_ID AND p.P_ID=te.P_ID and k.K_ID=t.K_ID and m.ME_ID=t.ME_ID and pr.T_ID=t.T_ID and t.T_ID = %s")
	cursor.execute(select_termek, [id])
	cnx.commit()
...
def lekerdezkosar():	
	...
	select_termek = ("SELECT k.T_ID, t.Nev, sz.Nev, Mennyiseg, me.Nev, pr.Ar, Penznem "
					" FROM Kosarak k, Termekek t, Szemelyek sz, Termelok te, Mertekegysegek me, Penznemek p, Promociok pr "
					" WHERE t.T_ID = k.T_ID AND t.SZ_ID=sz.SZ_ID AND sz.SZ_ID=te.SZ_ID AND p.P_ID=te.P_ID AND k.SZ_ID = %s AND t.ME_ID=me.ME_ID AND pr.T_ID=k.T_ID "
	...
	select_termek = ("SELECT k.T_ID, t.Nev, sz.Nev, Mennyiseg, me.Nev, t.Ar, Penznem "
					" FROM Kosarak k, Termekek t, Szemelyek sz, Termelok te, Mertekegysegek me, Penznemek p"
					" WHERE t.T_ID = k.T_ID AND t.SZ_ID=sz.SZ_ID AND sz.SZ_ID=te.SZ_ID AND p.P_ID=te.P_ID AND t.ME_ID=me.ME_ID AND k.SZ_ID = %s "
...
def termekmodositas():
	...
	string = ("UPDATE Termekek SET "
				"Nev = %s, Leiras = %s, Ar = %s, Min_rendelesi_menny = %s, Keszlet_menny = %s, ME_ID = %s, K_ID = %s, SZ_ID = %s "
				"WHERE T_ID = %s")
	datas = (adatok['nev'], adatok['leiras'], adatok['ar'], adatok['rend_menny'], adatok['keszlet_menny'], adatok['mertekegyseg'], adatok['kategoria'], session['SZ_ID'], adatok['id'])
...
def termekfeltoltes():
	...
	string = ("INSERT INTO Termekek "
				   "(Nev, Leiras, Ar, Min_rendelesi_menny, Kep, Keszlet_menny, ME_ID, K_ID, SZ_ID)"
				   "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)")

	datas = (adatok['nev'], adatok['leiras'], adatok['ar'], adatok['rend_menny'], 'termek.jpg', adatok['keszlet_menny'], adatok['ME_ID'], adatok['K_ID'], session['SZ_ID'])
...
def profilombetoltes():
	...
	select_telepulesek = ("SELECT Telep_ID, Nev FROM Telepulesek ORDER BY Nev")
	select_napok = ("SELECT N_ID, Nev FROM Napok")
	...
	cursor.execute(select_telepulesek)
	cnx.commit()
	ertekek3 = cursor.fetchall()
	telepulesek = []
	for t in ertekek3:
		telepulesek.append({'value':t[0], 'text':t[1]})
	logging.warning(telepulesek)
	
	cursor.execute(select_napok)
	cnx.commit()
	ertekek4 = cursor.fetchall()
	napok = []
	for n in ertekek4:
		napok.append({'value':n[0], 'text':n[1]})
	logging.warning(napok)
	...
	select_telepulesek_adatok = ("SELECT * FROM Kiszallitasi_helyek WHERE SZ_ID = %s")
	cursor.execute(select_telepulesek_adatok, [session['SZ_ID']])
	cnx.commit()
	telepules_adat = cursor.fetchall()
	
	logging.warning(telepules_adat)
	telep = []
	for t in telepules_adat:
		telep.append(t[2])
	logging.warning(telep)

	select_napok_adatok = ("SELECT * FROM Kiszallitasi_napok WHERE SZ_ID = %s")
	cursor.execute(select_napok_adatok, [session['SZ_ID']])
	cnx.commit()
	napok_adat = cursor.fetchall()
	
	logging.warning(napok_adat)
	nap = []
	for n in napok_adat:
		nap.append(n[2])
	logging.warning(nap)	
	...
	cursor.close()
	cnx.close()
	return jsonify({'profilom':profilom, 'penznemek':penznemek, 'rendszeresseg':rendszeresseg, 'profilom_adat':profilom_adat, 'telepulesek':telepulesek, 'napok':napok, 'telepules_adat':telep, 'nap_adat':nap})	
...
def profilommodositas():
	...
	if teszt_eredmeny is None :
		...
		for telepules in adatok['telepulesek']:
			insert_telepulesek = ("INSERT INTO Kiszallitasi_helyek (SZ_ID, Telep_ID) VALUES (%s, %s)")
			datas = (session['SZ_ID'], telepules)
			try:
				cursor.execute(insert_telepulesek, datas)
				cnx.commit()
			except:
				cnx.rollback()	
				
		#for nap in adatok['napok']:
			#insert_napok = ("INSERT INTO Kiszallitasi_napok (SZ_ID, N_ID) VALUES (%s, %s)")
			#datas = (session['SZ_ID'], nap)
			#try:
		...
		delete_telepules = ("DELETE FROM Kiszallitasi_helyek WHERE SZ_ID = %s")
		cursor.execute(delete_telepules, [session['SZ_ID']])
		cnx.commit()
		for telepules in adatok['telepulesek']:
			insert_telepulesek = ("INSERT INTO Kiszallitasi_helyek (SZ_ID, Telep_ID) VALUES (%s, %s)")
			datas = (session['SZ_ID'], telepules)
			logging.warning("telepulesek telepules:")
			logging.warning(datas)
			try:
				cursor.execute(insert_telepulesek, datas)
				cnx.commit()
		...
		#delete_nap = ("DELETE FROM Kiszallitasi_napok WHERE SZ_ID = %s")
		#cursor.execute(delete_telepules, [session['SZ_ID']])
		#cnx.commit()
		#for nap in adatok['napok']:
			#insert_napok = ("INSERT INTO Kiszallitasi_napok (SZ_ID, N_ID) VALUES (%s, %s)")
...
@app.route('/telepulesvizsgalat/', methods = ['POST'])
def telepulesvizsgalat():
	adatok = json.loads(request.data)
	cnx = mysql.connector.connect(user=conf.user, password=conf.password, host=conf.host, database=conf.database, buffered=True)
	cursor = cnx.cursor()
	
	telepules = ("SELECT * FROM Telepulesek WHERE Nev = %s")
	logging.warning("telepules:")
	logging.warning(adatok['telep'])
	
	cursor.execute(telepules, [adatok['telep']])
	cnx.commit()
...
@app.route('/telepulesmentes/', methods = ['POST'])
def telepulesmentes():
	adatok = json.loads(request.data)
	cnx = mysql.connector.connect(user=conf.user, password=conf.password, host=conf.host, database=conf.database, buffered=True)
	cursor = cnx.cursor()
	
	insert_telepules = ("INSERT INTO Telepulesek ( Nev ) VALUES ( %s ) ")
	try:
		cursor.execute(insert_telepules, [adatok['telep']])
		cnx.commit()
	except:
		cnx.rollback()
...
def promtermekekbetoltes():
	...
	select_promtermekek_regiadatai = ("SELECT T_ID, Termekek.Nev, Ar, Penznemek.P_ID, Penznem FROM Termekek, Penznemek, Termelok WHERE T_ID in (SELECT Promociok.T_ID FROM Termekek, Promociok WHERE Termekek.T_ID = Promociok.T_ID AND Termekek.SZ_ID = %s) AND Penznemek.P_ID = Termelok.P_ID AND Termelok.SZ_ID = %s ")
	...
	logging.warning("promtermekekuj:")
	logging.warning(promtermekekuj)
	...
	sorok_szama = len(promtermekek)
	logging.warning("sorok_szama")
	logging.warning(sorok_szama)
	...
	datas = (session['SZ_ID'], session['SZ_ID'])
	cursor.execute(select_promtermekek_regiadatai, datas)
	cnx.commit()
...
def promtermek_aktualizal():
	...
	select_promtermekek_regiadatai = ("SELECT T_ID, Termekek.Nev, Ar, Termelok.P_ID, Penznem FROM Termekek, Termelok, Penznemek WHERE Termekek.Nev = %s AND Termelok.P_ID = Penznemek.P_ID AND Termelok.SZ_ID = %s AND Termekek.SZ_ID = %s ")
	datas = (adatok['nev'], session['SZ_ID'], session['SZ_ID'])
	cursor.execute(select_promtermekek_regiadatai, datas)
...
@app.route('/promtermektorol/', methods = ['POST'])	
def promtermektorol():
	adatok = json.loads(request.data) 
	...
	delete_termek = ("DELETE FROM Promociok WHERE T_ID = %s")
	
	try:
		cursor.execute( delete_termek, [adatok['id']])
		cnx.commit()
		result = True
	except:
		cnx.rollback()
...
def login():
	...
	if not user:
		...
		cursor.close()
		cnx.close()
		logging.warning("nincs ilyen email cim")
